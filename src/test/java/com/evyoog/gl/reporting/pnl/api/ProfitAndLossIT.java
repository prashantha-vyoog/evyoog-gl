...
class ProfitAndLossIT {
    ...
    @Test
    void testPnl_afterPostingSalesAndExpenseJournals_correctTotals() throws Exception {
        Fixture fx = buildFixture();
        openPeriod(fx, fx.periodId);

        // Sales journal: DR AR 1000 / CR Sales(domestic) 1000
        postingEngine.post(multiLineRequest(fx,
                line(fx.arAccountId, "1000.00", null),
                line(fx.domesticSalesId, null, "1000.00")));

        // Expense journal: DR Raw Material 400 / CR Payable 400
        postingEngine.post(multiLineRequest(fx,
                line(fx.rawMaterialId, "400.00", null),
                line(fx.payableAccountId, null, "400.00")));

        String response = mockMvc.perform(get("/api/v1/gl/reports/profit-and-loss")
        ...
        assertThat(data.get("totalRevenue").decimalValue()).isEqualByComparingTo("1000.00");
        assertThat(data.get("totalExpenses").decimalValue()).isEqualByComparingTo("400.00");
        assertThat(data.get("netIncome").decimalValue()).isEqualByComparingTo("600.00");
        assertThat(data.get("isProfitable").asBoolean()).isTrue();
    }

    @Test
    void testPnl_hierarchyRollup_summaryAccountHasChildBalances() throws Exception {
        Fixture fx = buildFixture();
        openPeriod(fx, fx.periodId);

        postingEngine.post(multiLineRequest(fx,
                line(fx.arAccountId, "600.00", null),
                line(fx.domesticSalesId, null, "600.00")));
        postingEngine.post(multiLineRequest(fx,
                line(fx.arAccountId, "300.00", null),
                line(fx.exportSalesId, null, "300.00")));
        ...
        var revenueItems = objectMapper.readTree(response).at("/data/revenueItems");
        JsonNode revenueSummary = null;
        for (var node : revenueItems) {
            if (node.get("accountCode").asText().equals(fx.revenueSummaryCode)) {
                revenueSummary = node;
            }
        }
        assertThat(revenueSummary).isNotNull();
        assertThat(revenueSummary.get("children")).hasSize(2);
        assertThat(revenueSummary.get("netAmount").decimalValue()).isEqualByComparingTo("900.00");
    }

    @Test
    void testPnl_eventOnlyLedger_returns422() throws Exception {
        Fixture fx = buildFixture("EVENT_ONLY");

        mockMvc.perform(get("/api/v1/gl/reports/profit-and-loss")
                        .param("legalEntityId", fx.legalEntityId.toString())
                        .param("periodId", fx.periodId.toString()))
    ...
    @Test
    void testPnl_noJournalsPosted_returns404() throws Exception {
        Fixture fx = buildFixture();
    ...
    private Fixture buildFixture(String financeMode) throws Exception {
        ...
        String revenueSummaryCode = "4000-" + suffix;
        String domesticSalesCode = "4100-" + suffix;
        String exportSalesCode = "4200-" + suffix;
        String rawMaterialCode = "5100-" + suffix;

        UUID arId = createDimensionValue(naturalAcctDimId, arCode, "ASSET", "DR", null, false);
        UUID payableId = createDimensionValue(naturalAcctDimId, payableCode, "LIABILITY", "CR", null, false);
        UUID revenueSummaryId = createDimensionValue(naturalAcctDimId, revenueSummaryCode, "REVENUE", "CR", null, true);
        UUID domesticSalesId = createDimensionValue(naturalAcctDimId, domesticSalesCode, "REVENUE", "CR", revenueSummaryId, false);
        UUID exportSalesId = createDimensionValue(naturalAcctDimId, exportSalesCode, "REVENUE", "CR", revenueSummaryId, false);
        UUID rawMaterialId = createDimensionValue(naturalAcctDimId, rawMaterialCode, "EXPENSE", "DR", null, false);
        ...
        return new Fixture(legalEntityId, periodIds.get(0),
                arId, payableId, revenueSummaryCode, domesticSalesId, exportSalesId, rawMaterialId,
    ...
    private static final class Fixture {
        ...
        final String revenueSummaryCode;
        final UUID domesticSalesId;
        final UUID exportSalesId;
        final UUID rawMaterialId;
        final UUID journalSourceId;
        final UUID journalCategoryId;

        Fixture(UUID legalEntityId, UUID periodId,
                UUID arAccountId, UUID payableAccountId,
                String revenueSummaryCode, UUID domesticSalesId, UUID exportSalesId, UUID rawMaterialId,
                UUID journalSourceId, UUID journalCategoryId) {
            this.legalEntityId = legalEntityId;
            this.periodId = periodId;
            this.arAccountId = arAccountId;
            this.payableAccountId = payableAccountId;
            this.revenueSummaryCode = revenueSummaryCode;
            this.domesticSalesId = domesticSalesId;
            this.exportSalesId = exportSalesId;
