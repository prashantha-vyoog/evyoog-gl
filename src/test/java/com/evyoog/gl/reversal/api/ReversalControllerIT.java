...
class ReversalControllerIT {
    ...
    @Test
    void testFullReversalLifecycle_postJournal_reverse_bothStatusesCorrect() throws Exception {
        Fixture fx = buildFixture("THICK");
        openPeriod(fx);

        UUID journalId = createJournal(balancedRequest(fx));
        ...
        mockMvc.perform(post("/api/v1/gl/journals/{id}/reverse", journalId)
                ...
                .andExpect(jsonPath("$.data.originalJournalId").value(journalId.toString()))
                .andExpect(jsonPath("$.data.originalStatus").value("REVERSED"))
                .andExpect(jsonPath("$.data.reversalStatus").value("POSTED"));
    ...
    @Test
    void testReversal_linesAreFlipped_balancesMaintained() throws Exception {
        Fixture fx = buildFixture("THICK");
        openPeriod(fx);

        UUID journalId = createJournal(balancedRequest(fx));

        String reverseResponse = mockMvc.perform(post("/api/v1/gl/journals/{id}/reverse", journalId)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(Map.of(
                                "targetPeriodId", fx.periodId.toString(),
                                "reversedBy", "reverser1"))))
        ...
        UUID reversalJournalId = UUID.fromString(objectMapper.readTree(reverseResponse).at("/data/reversalJournalId").asText());

        mockMvc.perform(get("/api/v1/gl/journals/{id}", reversalJournalId))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.isReversal").value(true))
                .andExpect(jsonPath("$.data.totalDebit").value(100.00))
                .andExpect(jsonPath("$.data.totalCredit").value(100.00))
                .andExpect(jsonPath("$.data.lines[0].debitAmount").doesNotExist())
                .andExpect(jsonPath("$.data.lines[0].creditAmount").value(100.00))
                .andExpect(jsonPath("$.data.lines[1].debitAmount").value(100.00))
                .andExpect(jsonPath("$.data.lines[1].creditAmount").doesNotExist());
    }

    @Test
    void testReversal_alreadyReversed_returns409() throws Exception {
        Fixture fx = buildFixture("THICK");
        openPeriod(fx);

        UUID journalId = createJournal(balancedRequest(fx));

        Map<String, Object> reverseBody = Map.of("targetPeriodId", fx.periodId.toString(), "reversedBy", "reverser1");
        mockMvc.perform(post("/api/v1/gl/journals/{id}/reverse", journalId)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(reverseBody)))
                .andExpect(status().isCreated());

        // The source journal is already REVERSED after the first call, so the
        // second attempt fails the POSTED-status check before it ever reaches
        // the ALREADY_REVERSED guard — both are 409s either way.
        mockMvc.perform(post("/api/v1/gl/journals/{id}/reverse", journalId)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(reverseBody)))
                .andExpect(status().isConflict())
                .andExpect(jsonPath("$.code").value("INVALID_REVERSAL_STATE"));
    }

    @Test
    void testReversal_nonPostedJournal_returns409() throws Exception {
        Fixture fx = buildFixture("THICK");
        openPeriod(fx);
        ...
        mockMvc.perform(post("/api/v1/gl/journals/{id}/reverse", journalId)
                        ...
                        .content(objectMapper.writeValueAsString(Map.of(
                                ...
                                "reversedBy", "reverser1"))))
                .andExpect(status().isConflict())
                .andExpect(jsonPath("$.message").value(org.hamcrest.Matchers.containsString("Only POSTED journals can be reversed")));
    }

    @Test
    void testReversal_targetPeriodClosed_returns409() throws Exception {
        Fixture fx = buildFixture("THICK");
        openPeriod(fx);

        UUID journalId = createJournal(balancedRequest(fx));

        // The second period exists on the calendar but was never opened for
        // posting — validatePeriodOpen rejects it the same way it would a
        // CLOSED period.
        UUID unopenedPeriodId = secondPeriodId(fx.calendarId);

        mockMvc.perform(post("/api/v1/gl/journals/{id}/reverse", journalId)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(Map.of(
                                "targetPeriodId", unopenedPeriodId.toString(),
                                "reversedBy", "reverser1"))))
                .andExpect(status().isConflict())
                .andExpect(jsonPath("$.message").value(org.hamcrest.Matchers.containsString("period")));
    }

    @Test
    void testGetReversalDetails_returnsReversalPair() throws Exception {
        Fixture fx = buildFixture("THICK");
        openPeriod(fx);
        ...
        mockMvc.perform(get("/api/v1/gl/journals/{id}/reversal", journalId))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.originalJournalId").value(journalId.toString()))
                .andExpect(jsonPath("$.data.originalStatus").value("REVERSED"))
                .andExpect(jsonPath("$.data.reversalStatus").value("POSTED"));
    ...
    @Test
    void testGetReversalDetails_noReversal_returns404() throws Exception {
        Fixture fx = buildFixture("THICK");
        openPeriod(fx);
    ...
    private UUID secondPeriodId(UUID calendarId) throws Exception {
        String periodsResponse = mockMvc.perform(get("/api/v1/gl/accounting-calendars/{calendarId}/periods", calendarId))
                .andExpect(status().isOk())
                .andReturn().getResponse().getContentAsString();
        return UUID.fromString(objectMapper.readTree(periodsResponse).at("/data/1/id").asText());
    }
    ...
    private Fixture buildFixture(String financeMode) throws Exception {
        ...
        return new Fixture(legalEntityId, ledgerId, calendarId, periodId, naturalAcctDimId,
    ...
    private static final class Fixture {
        ...
        Fixture(UUID legalEntityId, UUID ledgerId, UUID calendarId, UUID periodId, UUID naturalAcctDimId,
